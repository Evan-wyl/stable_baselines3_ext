...
class AMPDiscriminator(nn.Module):
    ...
    def compute_grad_pen(self, expert_state, expert_next_state, lambda_=10):
        expert_data = th.cat([expert_state, expert_next_state], dim=-1)
        expert_data.requires_grad = True

        disc = self.amp_linear(self.trunk(expert_data))
        ones = th.ones(disc.size(), device=disc.device)
        grad = autograd.grad(
    ...
    def predict_amp_reward(self, state, next_state, normalizer=None):
        with th.no_grad():
            ...
            d = self.amp_linear(self.trunk(th.cat([state, next_state], dim=-1)))
            reward = self.amp_reward_coef * th.clamp(1 - (1 / 4) * th.square(d - 1), min=0)
